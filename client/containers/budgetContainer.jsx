...
const BudgetContainer = () => {
  ...
  const display = [];
  if (monsterCount === 0)
    display.push('Talk to the comBOT to look for stuff...');
    ...
    for (const queryMonster of monsterList) {
      ...
      if (queryMonster.armor_desc !== null)
        monsterAC += ' ' + queryMonster.armor_desc;
      let monsterHD = queryMonster.hit_dice;
      display.push(
        <li key={id.toString()}>
          {count} - {monsterName} ({monsterAlignment}) Type: {monsterType} CR:{' '}
          {monsterCR} AC: {monsterAC} HP: {monsterHD}
        </li>
      );
      count++;
      id++;
